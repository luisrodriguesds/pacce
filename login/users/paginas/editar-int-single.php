...
<?php 
			...
			if (isset($_POST['enviar'])) {
				...
				$form['npacce1'] 		= $int['npacce1'];
				$form['bol1'] 			= $int['bol1'];
				$form['nomeComp1']		= $int['nomeComp1'];
				$form['npacce2'] 		= GetPost('npacce2');
				$form['vagaInicial'] 	= GetPost('vagas');
				$form['vagaAtual'] 		= $int['vagaAtual'];
				...
				if (empty($form['titulo'])) {
					...
					if ($form['vagaInicial'] < $int['vagaInicial']) {
						$res = $int['vagaInicial'] - $form['vagaInicial'];
						$form['vagaAtual'] = $form['vagaAtual'] - $res;
					}else if($form['vagaInicial'] > $int['vagaInicial']){
						$res = $form['vagaInicial'] - $int['vagaInicial'];
						$form['vagaAtual'] = $form['vagaAtual'] + $res;
					}
					if (DBUpDate('int_cad', $form, "codigo = '".$int['codigo']."'") ){
						echo '
		                 <script>
...
?>
<div class="title"><h2>Editar Informações da Interação <?php echo $int['codigo']; ?></h2></div>
<div id="editar-ativ" class="form">
	 <form action="" method="post" enctype="multipart/form-data">
	 	Código<br><strong>
		<?php echo $int['codigo']; ?></strong><br><br>
		Título<br>
		<input type="text" name="titulo" value="<?php echo $int['titulo']; ?>" placeholder="Digite o título dessa interação"></input><br><br>
		Data<br>
		<input type="date" name="data" value="<?php echo date('Y-m-d', strtotime($int['data'])); ?>"></input><br><br>
		Início<br>
		<input type="time" name="inicio" value="<?php echo $int['inicio']; ?>"></input><br><br>
		Fim<br>
		<input type="time" name="fim" value="<?php echo $int['fim']; ?>"></input><br><br>
		Local:<br>
		<input type="text" name="local" onkeypress="masculina(this)" onkeyup="masculina(this)" value="<?php echo $int['local']; ?>" placeholder="Digite o local dessa interação"></input><br><br>
		Semana:<br>
		<strong><?php echo '0'.$int['semana']; ?></strong><br><br>
		Número Pacce do Co-facilitador<br>
		*Campo não obrigatório<br>
		<input type="text" name="npacce2" value="<?php echo $int['npacce2']; ?>" placeholder="Digite o número PACCE do Co-facilitador"></input>
		<br><br>
		Status:<br>
		<select name="status">
		...
		<br><br>
		Número de vagas<br>
		<input type="number" name="vagas" min="1" value="<?php echo $int['vagaInicial']; ?>" max="50" placeholder="Digite o número de vagas dessa interação"></input>
		Vaga Atual:<br>
		<strong><?php echo $int['vagaAtual']; ?></strong>
		<br><br>Cor do Card:<br>
		<input type="color" name="cor" value="<?php echo $int['cor']; ?>"></input><br><br>
		Descrição:<br>
