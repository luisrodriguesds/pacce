...
<?php 
		...
		if ($intCad == false) {
			...
			switch ($_GET['action']) {
				case 1:
					...
					$verify 	= DBread('int_insc', "WHERE npacce = '".$user['npacce']."' AND codigo = '$id'");
					if ($verify == true) {
						echo ' <script>
				   window.location="'.$way.'/'.$url[1].'/'.$url[2].'";
				   </script>';
						...
						//se nao, inseri no banco
							...
							for ($i=0; $i < count($intCad); $i++) { 
								if ($intCad[$i]['codigo'] == $id) {
									$conte++;
									$inscricao['npacce'] 	= $user['npacce'];
									$inscricao['nome']	 	= GetName($user['nome'], $user['nomeUsual']);	
									$inscricao['nomeCompleto']= $user['nome'];
									$inscricao['codigo'] 	= $intCad[$i]['codigo'];
									$inscricao['titulo'] 	= $intCad[$i]['titulo'];
									$inscricao['semana'] 	= $intCad[$i]['semana'];
									$up['vagaAtual']		= $intCad[$i]['vagaAtual'];
							...
							if ($conte > 0) {
								if (DBcreate('int_insc', $inscricao)) {
									$up['vagaAtual'] = $up['vagaAtual'] - 1;
									if (DBUpdate('int_cad', $up, "codigo = '$id'")) {
										echo ' <script>
										   window.location="'.$way.'/'.$url[1].'/'.$url[2].'";
										   </script>';
									}
				...
				$verify = DBread('int_insc', "WHERE npacce = '".$user['npacce']."' AND codigo = '$id'");
					if ($verify == true) {
						if (DBDelete('int_insc', "npacce = '".$user['npacce']."' AND codigo = '$id'")) {
							for ($i=0; $i < count($intCad); $i++) { 
								if ($intCad[$i]['codigo'] == $id) {
									$up['vagaAtual']		= $intCad[$i]['vagaAtual'];
								}
	...
	$intCad = DBread('int_cad', "WHERE codigo = '".$url[2]."' ");
	$inscritos = DBread('int_insc', "WHERE status = true AND codigo = '$url[2]' ORDER BY nome ASC");
	...
	if($intCad == false) {
		...
		for ($i=0; $i < count($intCad); $i++) { 
			if ($intCad[$i]['codigo'] == $url[2]) {
				$int['codigo'] 			= $intCad[$i]['codigo'];
				$int['titulo'] 			= $intCad[$i]['titulo'];
				$int['semana'] 			= $intCad[$i]['semana'];
				$int['local'] 			= $intCad[$i]['local'];
				$int['data'] 			= $intCad[$i]['data'];
				$int['inicio'] 			= $intCad[$i]['inicio'];
				$int['fim'] 			= $intCad[$i]['fim'];
				$int['diaSemana'] 		= $intCad[$i]['diaSemana'];
				$int['bol1'] 			= $intCad[$i]['bol1'];
				$int['bol2'] 			= $intCad[$i]['bol2'];
				$int['npacce1'] 		= $intCad[$i]['npacce1'];
				$int['npacce2'] 		= $intCad[$i]['npacce2'];
				$int['nomeComp1'] 		= $intCad[$i]['nomeComp1'];
				$int['nomeComp2'] 		= $intCad[$i]['nomeComp2'];
				$int['vagaAtual'] 		= $intCad[$i]['vagaAtual'];
				$int['vagaInicial'] 	= $intCad[$i]['vagaInicial'];
				$int['disc'] 			= $intCad[$i]['disc'];
				$int['status'] 			= $intCad[$i]['status'];
				$int['cor'] 			= $intCad[$i]['cor'];
			}
		}
		if (isset($_GET['editar']) && $_GET['editar'] != '') {
			if ($user['npacce'] == 'B140084' ||  $user['npacce'] == $int['npacce1'] || $user['npacce'] == $int['npacce2'] || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico') {
				include 'paginas/editar-int-single.php';
			}
		}else{
?>	
	...
	//===============Cadastrar em interação ================
		if (isset($_POST['send'])) {
			...
			$form['codigo'] 	= $int['codigo'];
			$form['semana'] 	= $int['semana'];
			$form['titulo'] 	= $int['titulo'];
			$form['status'] 	= 1;
			$form['situacao'] 	= 0;
			$up['vagaAtual']	= $int['vagaAtual'];
			...
			if ($bol == false) {
				...
				$intInsc = DBread('int_insc', "WHERE npacce = '".$form['npacce']."' AND codigo = '".$int['codigo']."'", "id");
				if ($intInsc == true) {
				 	...
				 	if ($up['vagaAtual'] <= 0) {
				 		...
				 		if (DBcreate('int_insc', $form)) {
					 		$up['vagaAtual'] = $up['vagaAtual'] - 1; 
					 		if (DBUpDate('int_cad', $up, "codigo = '".$int['codigo']."'")) {
	...
	function excluir(){
			if (confirm("Você tem certeza que desaja excluir essa sala?")){
				window.location="<?php echo $way.'/'.$url[1].'/'.$url[2].'?action=3&&cod='.$int['codigo']; ?>";
			}
		}	
	</script>
	<div class="page-ati-single">
		<div class="cabecario">
					<div class="info-single">
						<ul>
							<li><h3><?php echo $int['titulo']; ?></h3></li>
							<li><?php echo $int['local']; ?></li>

							<li><?php echo date('d/m', strtotime($int['data'])); ?> -  <?php echo $int['diaSemana']; ?></li>
							 <li><?php echo date('H:i', strtotime($int['inicio'])).'hr'; ?>
							  - <?php echo date('H:i', strtotime($int['fim'])).'hr'; ?> </li>

							<li>Semana <?php if($int['semana'] >= 10){echo $int['semana'];}else{echo '0'.$int['semana'];} ?></li>

							<li><?php echo $int['bol1']; if($int['bol2'] == ''){}else{echo ' & '.$int['bol2'];} ?></li>
							
							<li><?php echo $int['vagaAtual']; ?>/<?php echo $int['vagaInicial']; ?> Vagas</li>
							<li><p><strong> <?php echo printPost($int['disc'], 'page'); ?> </strong> </p></li>
						</ul>
					</div>
					
					<div class="foto-single">
						<?php $foto = DBread('bolsistas', "WHERE npacce = '".$int['npacce1']."'", "id, foto"); ?>
							<span class="foto"><img src="<?php echo URL_PAINEL.'/imagensBolsistas/'.$foto[0]['foto'].''; ?>"><br></span>
							<div class="nome"><?php echo $int['bol1']; ?></div>
					</div>
					<?php 
						if($int['bol2'] == ''){
						}else{
							$foto = DBread('bolsistas', "WHERE npacce = '".$int['npacce2']."'", "id, foto");
						?> 
						<div class="foto-single">
							<span class="foto"><img src="<?php echo URL_PAINEL.'/imagensBolsistas/'.$foto[0]['foto'].''; ?>"><br></span>
							<div class="nome"><?php echo $int['bol2']; ?></div>
						</div>
						<?php 
						}
					?>
					<div id="clear"></div>
					<?php
							if($user['npacce'] == $int['npacce1'] || $user['npacce'] == 'B160676' || $user['npacce'] == 'B150323' || $user['npacce'] == $int['npacce2'] || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico') {
								
								echo '<a href="'.URL_PAINEL.'paginas/int-lista-single.php?cod='.$int['codigo'].'" target="_blank" class="btn btn-primary">Lista de Presença</a>';
								
								if ($int['semana'] == $semanaProx) {
									
										$verify 	= DBread('int_insc', "WHERE npacce = '".$user['npacce']."' AND codigo = '".$url[2]."'");
								  		if ($verify == false) {
								  			if($int['vagaAtual'] > 0){
								  			echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=1&&ati='.$int['codigo'].'" class="btn btn-primary">Inscrever-se</a>';
								  			}
								  		}else{
								  			echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=2&&ati='.$int['codigo'].'" class="btn btn-primary">Cancelar Inscrição</a>';
								  		}
									
							  	}
							  	
								if ($int['status'] == 0) {
									echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=1&&cod='.$int['codigo'].'" class="btn btn-primary">Ativar</a>';
								}else{
									echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=2&&cod='.$int['codigo'].'" class="btn btn-primary">Desativar</a>';
								}
				
								echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?editar='.$int['codigo'].'" class="btn btn-primary">Editar</a>'; 
								echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'/remover-inscrito" class="btn btn-primary">Remover</a>'; 
								
		//=======================Cadastrar bolsista ==========================================//
								if ($user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico' || $user['npacce'] == $int['npacce1'] || $user['npacce'] == $int['npacce2']) {
		...
		<?php 
			if(isset($url[3]) && $url[3] == 'remover-inscrito'){
				if (isset($_GET['npacce']) && $_GET['npacce'] != '') {
					if (DBDelete('int_insc', "npacce = '".$_GET['npacce']."' AND codigo = '".$int['codigo']."'")) {
							
							$up['vagaAtual'] = $int['vagaAtual'] + 1;
							if (DBUpdate('int_cad', $up, "codigo = '".$int['codigo']."'")) {
								# code...
							}
		...
		//==================================Relatório================================================
				if ($user['npacce'] == $int['npacce1'] || $user['npacce'] == $int['npacce2']  || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico') {
					 	if (strtotime($data) > strtotime($int['data'])) {
					 		$ver = 0;
							for ($i=0; $i < count($inscritos); $i++) { 
	...
	<script type="text/javascript">
	 ...
	 <form action="" method="post" enctype="multipart/form-data">
		...
		?>
	 	...
	 	<input type="radio" name="presenca" class="presenca" <?php echo printRadio(GetPost('presenca'), '6'); ?> value="6"></input> Não<br><br>
 		<?php
	 	}
	 	if($select == true && $select[0]['situacao'] != 0){
		...
		<div class="corpo-single" id="corpo-single">
			...
			<?php 
				if ($inscritos == false) {
					...
					if ($user['npacce'] == $int['npacce2'] || $user['npacce'] == $int['npacce1'] || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico') {
					 	if (strtotime($data) > strtotime($int['data'])) {
