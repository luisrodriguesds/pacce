...
<?php 
	...
	if ($user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico' || $user['tipoSlug'] == 'apoio-interno') {
		$forSalas = DBread('for_salas', "WHERE sala = '".$url[2]."'");	
	}else{
		$forSalas = DBread('for_salas', "WHERE status = true AND sala = '".$url[2]."'");
	}
	$inscritos = DBread('for_insc', "WHERE sala = '".$url[2]."' ORDER BY nomeCompleto ASC");

	if ($forSalas == false) {
		echo '<div class="nada-encontrado"><h2>Nenhuma Formação encontrada.</h2></div>';
	}else{
		$for = $forSalas[0];

		if (isset($_GET['editar']) && $_GET['editar'] != '') {
			if ($user['npacce'] == $for['npacce1'] || $user['npacce'] == $for['npacce2']  || $user['npacce'] == $for['npacce3']  || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico' || $user['tipoSlug'] == 'apoio-interno') {
				include 'paginas/editar-for-single.php';
			}
		}else{
?>
<script>
function excluir(){
		if (confirm("Você tem certeza que desaja excluir essa sala?")){
			window.location="<?php echo $way.'/'.$url[1].'/'.$url[2].'?action=3&&sala='.$for['sala']; ?>";
		}
	}	
</script>
	<div class="page-ati-single">
		<div class="cabecario">
					<div class="info-single">
						<ul>
							<li><h3><?php echo $for['sala']; ?></h3></li>
							<li><?php echo $for['local']; ?></li>
							<li><?php echo date('d/m', strtotime($for['dataInicio'])); ?> - <?php echo date('d/m', strtotime($for['dataFim'])); ?></li>
							<li><?php echo date('H:i', strtotime($for['inicio'])).'hr'; ?> - <?php echo date('H:i', strtotime($for['fim'])).'hr'; ?><br> <?php echo $for['diaSemana']; ?> </li>
							<li><?php echo $for['bol1']; ?> & <?php echo $for['bol2']; ?> & <?php echo $for['bol3']; ?></li>
							<li><?php echo $for['vagaAtual']; ?>/<?php echo $for['vagaInicial']; ?> Vagas</li>
							<li><strong><?php echo printPost($for['disc'], 'page'); ?></strong></li>
							</ul>
					</div>
						

					<?php 
						if ($for['npacce3'] != '' || $for['npacce3'] != null) {
					?>

					<div class="foto-single">
					<?php 
						$foto = DBread('bolsistas', "WHERE npacce = '".$for['npacce3']."'", "id, foto"); ?>
							<span class="foto"><img src="<?php echo URL_PAINEL.'/imagensBolsistas/'.$foto[0]['foto'].''; ?>"><br></span>
							<div class="nome"><?php echo $for['bol3']; ?></div>
					</div>
					<?php } ?>	
						
					<?php 
						if ($for['npacce2'] != '' || $for['npacce2'] != null) {
					?>

					<div class="foto-single">
					<?php 
						$foto = DBread('bolsistas', "WHERE npacce = '".$for['npacce2']."'", "id, foto"); ?>
							<span class="foto"><img src="<?php echo URL_PAINEL.'/imagensBolsistas/'.$foto[0]['foto'].''; ?>"><br></span>
							<div class="nome"><?php echo $for['bol2']; ?></div>
					</div>
					<?php } ?>
					<div class="foto-single">
					<?php $foto = DBread('bolsistas', "WHERE npacce = '".$for['npacce1']."'", "id, foto"); ?>
							<span class="foto"><img src="<?php echo URL_PAINEL.'/imagensBolsistas/'.$foto[0]['foto'].''; ?>"><br></span>
							<div class="nome"><?php echo $for['bol1']; ?></div>
					</div>
					...
					<?php 
								if ($user['tipoSlug'] == 'articulador-de-celula') {
									if ($semanaFecha == true) {
											$verify = DBread('for_insc', "WHERE npacce = '".$user['npacce']."' AND sala = '".$for['sala']."' AND status = true", "id, sala");
											if ($verify == false) {
												if($for['vagaAtual'] > 0){
													echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=1&&ati='.$for['sala'].'" class="botao">Inscrever-se</a>';
												}
											}else{
												if ($for['sala'] == $verify[0]['sala']) {
													echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=2&&ati='.$for['sala'].'" class="botao">Cancelar Inscrição</a>';
												}
											}
										
									}
								}else if ($user['npacce'] == $for['npacce1'] || $user['npacce'] == $for['npacce2']  || $user['npacce'] == $for['npacce3']  || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico'){
									echo '<a href="'.URL_PAINEL.'paginas/for-lista-single.php?sala='.$for['sala'].'" target="_blank" class="botao">Lista de Presença</a>';
									if ($user['tipoSlug'] == 'ceo') {
									if ($for['status'] == 0) {
										echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=1&&sala='.$for['sala'].'" class="botao">Ativar</a>';
									}else{
										echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=2&&sala='.$for['sala'].'" class="botao">Desativar</a>';
									}
									echo '<a onclick="excluir()" style="cursor:pointer;" class="botao">Excluir</a>';
									}
									echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?editar='.$for['sala'].'" class="botao">Editar</a>';	
								}
				...
				if ($user['npacce'] == $for['npacce1'] || $user['npacce'] == $for['npacce2'] || $user['npacce'] == $for['npacce3'] || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico' || $user['tipoSlug'] == 'apoio-interno'){
					...
					if (isset($_POST['enviar'])) {
						...
						if ($form['presenca'] == '') {
							...
							echo '<script>alert("Campo Habilidades está vazio");</script>';
						}else if (empty($form['responsabilidade'])) {
							echo '<script>alert("Campo Responsabilidade está vazio");</script>';
						}else if (empty($form['contribuicao'])) {
							echo '<script>alert("Campo Contribuição está vazio");</script>';
						}else if (empty($form['protagonismo'])) {
							echo '<script>alert("Campo Protagonismo está vazio");</script>';
						}else{
							if ($select == false) {
								if (DBcreate($tabela, $form)) {
								echo '<script>alert("Relatório enviado com sucesso.");
...
//========================================Lista de relatórios=====================================
				if ($user['npacce'] == $for['npacce1'] || $user['npacce'] == $for['npacce2'] ||  $user['npacce'] == $for['npacce3'] || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico' || $user['tipoSlug'] == 'apoio-interno'){
				$forCod = DBread('for_cod', "WHERE codigo = '".$url[3]."' AND sala = '".$url[2]."'");
				if ($forCod == false) {
...
//========================================Inscritos na sala=======================================
	...
	<?php 
		//=============================Relatórios ================================//
		if ($user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico' || $user['npacce'] == $for['npacce1'] || $user['npacce'] == $for['npacce2'] || $user['npacce'] == $for['npacce3']) {
	?>
	<br><br>
<div class="title"><h2>Presenças</h2></div>
<?php 
	$forCod = DBread('for_cod', "WHERE sala = '".$for['sala']."' AND status = true");
	if ($forCod == false) {
...
<div class="tabela">
	<table border="">
		...
		<?php
			...
			//$dia = 5;
			//echo date('d-m-Y', strtotime('+'.$dia.' week', strtotime($for['dataInicio'])));
			$cn = 0;
			for ($i=0; $i < count($forCod); $i++) { 
				if (date('Y-m-d H:i:s', strtotime('+'.$i.' week', strtotime($for['dataInicio']))) < date('Y-m-d H:i:s')) {
					//echo date('Y-m-d H:i:s', strtotime('+'.$i.' week', strtotime($for['dataInicio']))).'<br>'.date('Y-m-d H:i:s').'<br>';
					$cn++;
		?>
		<tr>
			<td><?php echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'/'.$forCod[$i]['codigo'].'">'.$forCod[$i]['codigo'].'</a>'; ?></td>
			<td>
			<?php 
				echo substr($forCod[$i]['codigo'], -4, 2);
			 ?>
			 </td>
