...
<?php 
	...
	if ($user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico' || $user['tipoSlug'] == 'apoio-interno') {
		$apoSalas = DBread('apo_salas', "WHERE sala = '".$url[2]."'");	
	}else{
		$apoSalas = DBread('apo_salas', "WHERE status = true AND sala = '".$url[2]."'");
	}
	$inscritos = DBread('apo_insc', "WHERE sala = '".$url[2]."' ORDER BY nomeCompleto ASC");
	...
	if ($apoSalas == false) {
		...
		if (isset($_GET['editar']) && $_GET['editar'] != '') {
			if ($user['npacce'] == $apo['npacce1'] || $user['npacce'] == $apo['npacce2']  || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico' || $user['tipoSlug'] == 'apoio-interno') {
				include 'paginas/editar-apo-single.php';
			}
		}else{
...
function excluir(){
		if (confirm("Você tem certeza que desaja excluir essa sala?")){
			window.location="<?php echo $way.'/'.$url[1].'/'.$url[2].'?action=3&&sala='.$apo['sala']; ?>";
		}
	}	
</script>
	<div class="page-ati-single">
		<div class="cabecario">
					<div class="info-single">
						<ul>
							<li><h3><?php echo $apo['sala']; ?></h3></li>
							<li><?php echo $apo['local']; ?></li>
							<li><?php echo date('d/m', strtotime($apo['dataInicio'])); ?> - <?php echo date('d/m', strtotime($apo['dataFim'])); ?></li>
							<li><?php echo date('H:i', strtotime($apo['inicio'])).'hr'; ?> - <?php echo date('H:i', strtotime($apo['fim'])).'hr'; ?><br> <?php echo $apo['diaSemana']; ?> </li>
							<li><?php echo $apo['bol1']; if(!empty($apo['bol2'])) echo ' & '.$apo['bol2']; ?></li>
							<li><?php echo $apo['vagaAtual']; ?>/<?php echo $apo['vagaInicial']; ?> Vagas</li>
							<li><strong><?php echo printPost($apo['disc'], 'page'); ?></strong></li>
							
						</ul>
					</div>
					<?php 
						if ($apo['npacce2'] != '' || $apo['npacce2'] != null) {
					?>
					<div class="foto-single">
					<?php $foto = DBread('bolsistas', "WHERE npacce = '".$apo['npacce2']."'", "id, foto"); ?>
							<span class="foto"><img src="<?php echo URL_PAINEL.'/imagensBolsistas/'.$foto[0]['foto'].''; ?>"><br></span>
							<div class="nome"><?php echo $apo['bol2']; ?></div>
					</div>
					<?php } ?>
					<div class="foto-single">
					<?php $foto = DBread('bolsistas', "WHERE npacce = '".$apo['npacce1']."'", "id, foto"); ?>
							<span class="foto"><img src="<?php echo URL_PAINEL.'/imagensBolsistas/'.$foto[0]['foto'].''; ?>"><br></span>
							<div class="nome"><?php echo $apo['bol1']; ?></div>
					</div>
					...
					<?php 
								if ($user['tipoSlug'] == 'articulador-de-celula') {
									if ($semanaFecha == true) {
											$verify = DBread('apo_insc', "WHERE npacce = '".$user['npacce']."' AND status = true", "id, sala");
											if ($verify == false) {
												if($apo['vagaAtual'] > 0){
												echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=1&&ati='.$apo['sala'].'" class="botao">Inscrever-se</a>';
												}
											}else{
												if ($apo['sala'] == $verify[0]['sala']) {
													echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=2&&ati='.$apo['sala'].'" class="botao">Cancelar Inscrição</a>';
												}
											}
										
									}
								}else if ($user['npacce'] == $apo['npacce1'] || $user['npacce'] == $apo['npacce2']  || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico'){
									echo '<a href="'.URL_PAINEL.'paginas/apo-lista-single.php?sala='.$apo['sala'].'" class="botao">Lista de Presença</a>';
									if ($user['tipoSlug'] == 'ceo') {
									if ($apo['status'] == 0) {
										echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=1&&sala='.$apo['sala'].'" class="botao">Ativar</a>';
									}else{
										echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?action=2&&sala='.$apo['sala'].'" class="botao">Desativar</a>';
									}
									echo '<a onclick="excluir()" style="cursor:pointer;" class="botao">Excluir</a>';
									}
									echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'?editar='.$apo['sala'].'" class="botao">Editar</a>';	
								}
...
//========================================Formularios=============================================
				if ($user['npacce'] == $apo['npacce1'] || $user['npacce'] == $apo['npacce2'] || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico'){
					$ver = 0;
...
//========================================Lista de relatórios=====================================
				if ($user['npacce'] == $apo['npacce1'] || $user['npacce'] == $apo['npacce2'] || $user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico'){
				$apoCod = DBread('apo_cod', "WHERE codigo = '".$url[3]."' AND sala = '".$url[2]."'");
				if ($apoCod == false) {
...
//========================================Inscritos na sala=======================================
	...
	<?php 
		//=============================Relatórios ================================//
		if ($user['tipoSlug'] == 'ceo' || $user['npacce'] == $apo['npacce1'] || $user['npacce'] == $apo['npacce2'] || $user['tipoSlug'] == 'apoio-tecnico' || $user['tipoSlug'] == 'apoio-interno') {
	?>
	<br><br>
<div class="title"><h2>Presenças</h2></div>
<?php 
	$apoCod = DBread('apo_cod', "WHERE sala = '".$apo['sala']."' AND status = true ORDER BY codigo ASC");
	if ($apoCod == false) {
...
<div class="tabela">
	<table border="">
		...
		<?php
			...
			//$dia = 5;
			//echo date('d-m-Y', strtotime('+'.$dia.' week', strtotime($apo['dataInicio'])));
			$cn = 0;
			for ($i=0; $i < count($apoCod); $i++) { 
				if (date('Y-m-d H:i:s', strtotime('+'.$i.' week', strtotime($apo['dataInicio']))) < date('Y-m-d H:i:s')) {
					//echo date('Y-m-d H:i:s', strtotime('+'.$i.' week', strtotime($apo['dataInicio']))).'<br>'.date('Y-m-d H:i:s').'<br>';
					$cn++;
		?>
		<tr>
			<td><?php echo '<a href="'.$way.'/'.$url[1].'/'.$url[2].'/'.$apoCod[$i]['codigo'].'">'.$apoCod[$i]['codigo'].'</a>'; ?></td>
			<td>
			<?php 
				echo substr($apoCod[$i]['codigo'], -4, 2);
			 ?>
			 </td>
