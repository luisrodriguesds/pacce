...
<?php  
  ...
  if (isset($_GET['sala']) && $_GET['sala'] != '') {
  	...
  	$forSala = DBread('for_salas', "WHERE sala = '$sala'");
  	if ($forSala == false) {
  		Redirect($way.'/'.$sala);
  	}else if($forSala[0]['npacce1'] == $user['npacce']){
  	}else if($user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-interno' || $user['tipoSlug'] == 'apoio-tecnico'){
  	}else{
  		Redirect($way.'/'.$sala);
  	}

  	$forInsc = DBread('for_insc', "WHERE sala = '$sala' ORDER BY nomeCompleto ASC");
?>
<!DOCTYPE html>
<html>
<head>
	...
	<div id="folha-corpo">
		<div id="folha-cabecario">
			...
			<table>
				<tr>
					<th>Código:</th>
					<td><?php echo 'FOR'.substr($forSala[0]['ano'], -2).'___'.substr($forSala[0]['sala'], -2); ?></td>
					<th class="espaco">Dia:</th>
					<td><?php echo $forSala[0]['diaSemana']; ?></td>
					
				</tr>
				<tr>
					<th>Sala:</th>
					<td><?php echo $forSala[0]['sala']; ?></td>
					<th class="espaco">Local:</th>
					<td><?php echo $forSala[0]['local']; ?></td>
					
				</tr>
				<tr>
					<th>Horário:</th>
					<td><?php echo date('H:i', strtotime($forSala[0]['inicio'])).'hr - '.date('H:i', strtotime($forSala[0]['fim'])).'hr'; ?></td>
					<th class="espaco">Facilitador:</th>
					<td><?php echo $forSala[0]['npacce1']; ?> - <?php echo $forSala[0]['nomeComp1']; ?></td>
					
				</tr>
				<tr>
					<th>Data:</th>
					<td><?php echo date('d/m/y', strtotime($forSala[0]['dataInicio'])); ?> - <?php echo date('d/m/y', strtotime($forSala[0]['dataFim'])); ?></td>
					<th class="espaco">Co-Facilitador:</th>
					<td><?php echo $forSala[0]['npacce2']; ?> - <?php echo $forSala[0]['nomeComp2']; ?></td>
				</tr>
				<?php
				if ($forSala[0]['npacce3'] != '') {
					?>
				<tr>
					<th></th>
					<td></td>
					<th class="espaco">Co-Facilitador:</th>
					<td><?php echo $forSala[0]['npacce3']; ?> - <?php echo $forSala[0]['nomeComp3']; ?></td>
				</tr>
				<?php
				}
		...
		<table>
			...
			?>>
				<td><?php echo $forInsc[$i]['npacce']; ?></td>
				<td><?php echo $forInsc[$i]['nomeCompleto']; ?></td>
