...
<?php 
	if ($user['tipoSlug'] == 'ceo' || $user['tipoSlug'] == 'apoio-tecnico' || $user['tipoSlug'] == 'apoio-interno') {
		$apoSalas = DBread('apo_salas');
	}else{
		$apoSalas = DBread('apo_salas', "WHERE npacce1 = '".$user['npacce']."' OR npacce2 = '".$user['npacce']."'");
	}
	//ALTERAR STATUS
	if (isset($_GET['action']) && $_GET['action'] != '' && isset($_GET['sala']) && $_GET['sala'] != '') {
		$sala = DBescape(trim(strip_tags($_GET['sala'])));
		$very = DBread('apo_salas', "WHERE sala = '$sala'");
		if ($very == false) {
			echo ' <script>
		...
		switch ($_GET['action']) {
			case 1:
				$up['status'] = 1;
				if (DBUpDate('apo_salas', $up, "sala = '$sala'")) {
					if (DBUpDate('apo_insc', $up, "sala = '$sala'")) {
					echo ' <script>
			...
			break;
			case 3:
				if (DBDelete('apo_salas', "sala = '$sala'")) {
					if (DBDelete('apo_cod', "sala = '$sala'")) {
						if (DBDelete('apo_insc', "sala = '$sala'")) {
							echo ' <script>
			                  alert("Ação realizada com sucesso!");
...
?>
<div class="title"><h2>Gerenciar Apoio à Célula</h2></div>
...
<br><br>
<?php 
	if (isset($url[2]) && $url[2] == 'cadastrar-apoio-a-celula') {
	//====================CADASTRAR FORMAÇÃO===================
		if (isset($_POST['cadastrar'])) {
			...
			$sala 		= DBread('apo_salas', "WHERE sala = '".$form['sala']."'", "id");
	...
	<div class="title"><h2>Cadastrar Apoio à Célula</h2></div>
	<div id="editar-ativ" class="form">
		 <form action="" method="post" enctype="multipart/form-data">
       	...
       	<select name="sala">
       		<?php
       		$checkSalas = DBread('apo_salas', null, "sala");
       		...
       		for ($i=1; $i < 100 ; $i++) {
       			...
       			for ($j=0; $j < count($checkSalas); $j++) { 
       				if ($k == str_replace("A-", "", $checkSalas[$j]['sala'])) {
       					$v = 1;
       				}
       			}
       			...
       			echo '<option '.$block.' value="A-'.$k.'">'.$k.'</option>';
       		}
       		?>
       	</select>
       	<br><br>
...
?>
<div class="title"><h2><a href="<?php echo $way.'/'.$url[1].'/cadastrar-apoio-a-celula'; ?>">Cadastrar Apoio à Célula</a></h2></div>
...
<div class="caixa">
	...
	<a href="<?php echo $way.'/'.$url[1].'/'.$apoSalas[$i]['sala']; ?>">
		<li style="background-color: <?php if($apoSalas[$i]['status'] == 1){ echo $apoSalas[$i]['cor']; }else{ echo '#ccc'; }  ?>">
			<div>
				<div id="box-all-info">
					<div id="all-info">
						...
						<span><?php echo $apoSalas[$i]['bol1']; ?> & <?php echo $apoSalas[$i]['bol2']; ?></span><br>
